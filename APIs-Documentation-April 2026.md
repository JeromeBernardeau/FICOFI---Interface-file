...
6. [Paid Reserve Entry API](#paid-reserve-entry-api)
7. [Address APIs](#address-apis)
...
| type | Enum `Item Type` | Item type (Inventory, Service, Non-Inventory) | ✅ |
...
| tvtVintage | Code[10] | Wine vintage year | ✅ |
| tvtAlcVerified | Boolean | Alcohol % vol. verification flag (`TVTFIC Alc. % Vol. Verified`) | ✅ |
| grossWeight | Decimal | Gross weight | ✅ |
| netWeight | Decimal | Net weight | ✅ |
| unitVolume | Decimal | Unit volume | ✅ |
| tariffNo | Code[20] | Tariff number | ✅ |
| replenishmentSystem | Enum `Replenishment System` | Replenishment strategy | ✅ |
| itemCategoryCode | Code[20] | Item category code | ✅ |
| tvtAlcPercVol | Decimal | Alcohol percentage by volume | ✅ |
| tvtItemGroupCode | Code[20] | Item group code | ✅ |
| tvtficBundle | Boolean | Indicates whether the item is a bundle | ✅ |
...
**Description**: Applies an item template to the current item record.

| Aspect | Value |
|--------|-------|
| **Parameters** | `TemplateCode: Code[20]` |
| **Return Type** | None (void) |
| **Notes** | If `TemplateCode` is empty, the procedure loads the default template from `TVTFI Ficofi Setup` and applies it using `Item Templ. Mgt.`; the item is then modified. |
...
### Paid Reserve Entry API

![Version v2.0](https://img.shields.io/badge/version-v2.0-0A7A42)
![Read Only](https://img.shields.io/badge/CRUD-Read%20Only-1F6FEB)

**Entity Name**: `paidReserveEntryFic`
**Entity Set Name**: `paidReserveEntriesFic`
**Page ID**: 81019

#### Purpose
Provides read-only ledger-level visibility of paid reserve movements, balances, booking status, cost/price values, and delivery metadata.
...
| Field Name | Type | Description | Writable |
|---|---|---|---|
| id | Guid | System record identifier (OData key) | ❌ |
| entryNo | Integer | Paid reserve ledger entry number | ❌ |
| itemNo | Code[20] | Item number | ❌ |
| postingDate | Date | Posting date | ❌ |
| entryType | Enum `TVT Paid Reserve Entry Type` | Ledger entry classification | ❌ |
| paidReserveNo | Code[20] | Paid reserve number | ❌ |
| documentNo | Code[20] | Source document number | ❌ |
| description | Text[100] | Entry description | ❌ |
| locationCode | Code[10] | Location code | ❌ |
| quantity | Decimal | Entry quantity | ❌ |
| open | Boolean | Indicates whether the entry is open | ❌ |
| quantityBase | Decimal | Quantity in base unit of measure | ❌ |
| remQuantityBase | Decimal | Remaining quantity in base unit of measure | ❌ |
| unitCost | Decimal | Unit cost | ❌ |
| totalCost | Decimal | Total cost | ❌ |
| unitPrice | Decimal | Unit price | ❌ |
| totalPrice | Decimal | Total price | ❌ |
| sourceName | Text[100] | Source name | ❌ |
| reasonCode | Code[10] | Reason code | ❌ |
| sourceType | Enum `TVT PR Jnl Line Source Type` | Source type | ❌ |
| sourceNo | Code[20] | Source number | ❌ |
| provenanceSource | Text[100] | Provenance/source details | ❌ |
| condition | Text[100] | Condition text | ❌ |
| origPaidReserveNo | Code[20] | Original paid reserve number | ❌ |
| rotationNo | Code[50] | Rotation number | ❌ |
| bestBeforeDate | Date | Best-before date | ❌ |
| rotationRef | Code[50] | Rotation reference | ❌ |
...
| systemCreatedBy | Guid | System creator user ID | ❌ |
| systemModifiedAt | DateTime | System modification timestamp | ❌ |
| systemModifiedBy | Guid | System modifier user ID | ❌ |
| documentDate | Date | Document date | ❌ |
| externalDocumentNo | Code[35] | External document number | ❌ |
| documentType | Enum `Item Ledger Document Type` | Document type | ❌ |
| documentLineNo | Integer | Document line number | ❌ |
| internalInformation | Text[100] | Internal information | ❌ |
| itemLedgerEntryNo | Integer | Related item ledger entry number | ❌ |
| expDeliveryInstruction | Enum `TVT Exp. Delivery Instr.` | Expected delivery instruction | ❌ |
| expDeliveryShipTo | Code[10] | Expected delivery ship-to code | ❌ |
| contactNomineeNo | Code[20] | Contact nominee number | ❌ |
| contactNomineeName | Text[100] | Contact nominee name | ❌ |
| applyToILENo | Integer | Apply-to item ledger entry number | ❌ |
| tvtfiExpDeliveryLocCode | Code[10] | Expected delivery location code | ❌ |
| tvtfiReqPaidResDelDate | Date | Requested paid reserve delivery date | ❌ |
| tvtfiBaseUnitPrice | Decimal | Calculated base unit price (`unitPrice / qtyPerUnitOfMeasure`) | ❌ |
...
- [UPR (Unpaid Reserves) APIs](#upr-unpaid-reserves-apis)
